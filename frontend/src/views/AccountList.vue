...
function openCreate() {
  ...
  Object.assign(form, { id: null, name: '', balance: 0, description: '' })
...
async function submitForm() {
  ...
  const payload = { name: form.name, balance: form.balance, description: form.description }
  try {
    if (form.id) await api.put(`/api/accounts/${form.id}/`, payload)
...
async function handleDelete(row) {
  try {
    ...
    await api.delete(`/api/accounts/${row.id}/`)
    await fetchList()
